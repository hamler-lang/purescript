...
everywhereOnValues f g h = (f', g', h')
  where
    ...
    g' (Receive e1 e2 alts) = g (Receive (g' e1) (g' e2) (fmap handleCaseAlternative alts))
...
everywhereOnValuesTopDownM f g h = (f' <=< f, g' <=< g, h' <=< h)
  where
    ...
    g' (Receive e1 e2 alts) = Receive <$> (g' <=< g $ e1) <*> (g' <=< g $ e2) <*> traverse handleCaseAlternative alts
...
everywhereOnValuesM f g h = (f', g', h')
  where
    ...
    g' (Receive e1 e2 alts) = (Receive <$>  g' e1 <*> g' e2 <*> traverse handleCaseAlternative alts) >>= g
...
everythingOnValues (<>.) f g h i j = (f', g', h', i', j')
  where
    ...
    g' v@(Receive e1 e2 alts) = foldl (<>.) (foldl (<>.) (g v) (fmap g' [e1,e2])) (fmap i' alts)
...
everythingWithContextOnValues s0 r0 (<>.) f g h i j = (f'' s0, g'' s0, h'' s0, i'' s0, j'' s0)
  where
    ...
    g' s (Receive e1 e2 alts) = foldl (<>.) (foldl (<>.) r0 (fmap (g'' s) [e1, e2])) (fmap (i'' s) alts)
...
everywhereWithContextOnValuesM s0 f g h i j = (f'' s0, g'' s0, h'' s0, i'' s0, j'' s0)
  where
    ...
    g' s (Receive e1 e2 alts) = Receive <$> g'' s e1 <*> g'' s e2 <*> traverse (i'' s) alts
...
everythingWithScope f g h i j = (f'', g'', h'', i'', \s -> snd . j'' s)
  where
    ...
    g' s (Receive e1 e2 alts) = foldMap (g'' s) [e1, e2] <> foldMap (i'' s) alts
